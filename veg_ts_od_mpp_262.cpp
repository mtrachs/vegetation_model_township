...
namespace veg_pp {
  ...
  private:
    ...
    row_vector_d QT;
  public:
    veg_pp_model(stan::io::var_context& context__,
		 std::ostream* pstream__ = 0)
      : prob_grad(0) {
      // static const char* function__ = "veg_stan_od_mpp_MT_model_namespace::veg_stan_od_mpp_MT_model";
      ...
      validate_non_negative_index("y", "N_township", N_township);
	...
	for (size_t m_mat__ = 0; m_mat__ < d_inter_m_mat_lim__; ++m_mat__) {
      ...
      validate_non_negative_index("TS_matrix", "N", N);
      validate_non_negative_index("TS_matrix", "N_township", N_township);
	...
	throw std::domain_error(std::string("Invalid value of N_knots: ") + std::string(e.what()));
      ...
      try { 
	check_greater_or_equal(function__,"N_township",N_township,0);
      } catch (const std::exception& e) { 
	throw std::domain_error(std::string("Invalid value of N_township: ") + std::string(e.what()));
      };
      try { 
	...
	throw std::domain_error(std::string("Invalid value of N_p: ") + std::string(e.what()));
      ...
      validate_non_negative_index("ones", "N", N);
      ones = vector_d(N);
	...
	for (int i = 1; i <= N_knots; ++i) {
	  stan::math::assign(get_base1_lhs(eye_knots,i,j,"eye_knots",1), 0.0);
	}
	stan::math::assign(get_base1_lhs(eye_knots,j,j,"eye_knots",1), 1.0);
      }
      // for (int j = 1; j <= N; ++j) {
      // 	for (int i = 1; i <= N; ++i) {
	...
	eta(j1__) = vals_r__[pos__++];
      try { writer__.vector_lub_unconstrain(0.10000000000000001,sqrt(100),eta); } catch (const std::exception& e) {  throw std::runtime_error(std::string("Error transforming variable eta: ") + e.what()); }
	...
	for (int i0__ = 0U; i0__ < K; ++i0__)
      ...
      for (int i0__ = 0U; i0__ < K; ++i0__)
	try { writer__.vector_unconstrain(alpha[i0__]); } catch (const std::exception& e) {  throw std::runtime_error(std::string("Error transforming variable alpha: ") + e.what()); }
	...
	for (int k = 0; k < K; ++k) {
	  r_township(nt,k) = 0.0;
	  for (int i = 0; i < N; ++i) {
	    //std::cout << "r = " << r.row(i).sum() <<  std::endl;
	    //std::cout << "TS_matrix" << TS_matrix(i,nt) <<  std::endl;
	    //std::cout << "r_township = " << r_township(nt,k) <<  std::endl;
	...
	for (int m=0; m<K; m++) {
	  ...
	  for (int t=0; t<N_township; t++){
	    gradient[3*K+K*N_knots+k*N+i] += y[t][m] / r_township(t,m) * TS_matrix(i,t) * drdg;
	  }
	}
      }
      }
    ...
    void write_csv(RNG& base_rng__,
      ...
      stan::io::reader<double> in__(params_r__,params_i__);
      stan::io::csv_writer writer__(o__);
      static const char* function__ = "veg_pp::write_csv";
      (void) function__; // dummy call to supress warning
      // read-transform, write parameters
      vector_d eta = in__.vector_lub_constrain(0.10000000000000001,sqrt(100),K);
      writer__.write(eta);
