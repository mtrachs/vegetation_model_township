...
rows.use1 <- matrix(ncol=1,nrow=nrow(veg_coords))

for (xxx in 1:nrow(veg_coords)){
  rows.use1[xxx] <- which((coordinates.tot[,1]%in% veg_coords[xxx,1]) & (coordinates.tot[,2]%in%veg_coords[xxx,2]))
}
...
saveRDS(weight.neus.d,file='~/workflow_stepps_calibration/vegetation/data_township/township_weights.RDS')
